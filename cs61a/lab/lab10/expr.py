...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        if self.string in env:
            ...
            raise NameError("Error: "+ self.string +" not defined in env.")
    ...
    def eval(self, env):
        ...
        for i in self.operands:
            ...
            if type(i) is Name:
                ...
                if type(k) is Number:
                    opands.append(k)
                if type(k) is Literal:
                    opands.append(k.eval(env))
                if type(k) is PrimitiveFunction:
                    opands.append(k.apply(env))
                if type(k) is CallExpr:
                    opands.append(k.eval(env))
            elif type(i) is CallExpr:
                opands.append(i.eval(env))
            elif type(i) is Literal:
                opands.append(i.eval(env))
            else:
                opands.append(i)
        return ops.apply(opands)
