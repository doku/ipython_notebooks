...
def add_trees(t1, t2):
    ...
    diff = len(t1.branches) - len(t2.branches)
    if diff > 0:
        t2.branches = t2.branches + [Tree(0)] * diff
    if diff < 0:
        t1.branches = t1.branches + [Tree(0)] * abs(diff)
    branch = [add_trees(b1, b2) for b1, b2 in zip(t1.branches, t2.branches)]
