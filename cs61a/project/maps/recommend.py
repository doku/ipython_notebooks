...
def find_closest(location, centroids):
    ...
    """
    # BEGIN Question 3
    #f = lambda x: distance(location, x)
    return min(centroids, key=lambda x: distance(location, x) )
    """
    closest = centroids[0]
    closest_d = distance(location, centroids[0])
    for i in centroids:
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(x) for x in centroids]
        # END Question 6
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    meanx= mean(xs)
    meany= mean(ys)
    #sxx, syy = [[xi-meanx, yi-meany] for xi,yi zip(xs, ys)]
    sxx = sum([(xi-meanx)**2 for xi in xs])
    syy = sum([(yi-meany)**2 for yi in ys])
    sxy = sum([(xs[i]-meanx)*(ys[i]-meany) for i in range(len(xs))])
    #sxx = sxx**2
    #syy = syy**2
    b = sxy/sxx
    a, r_squared = meany-b*meanx, sxy**2 / (sxx*syy)  # REPLACE THIS LINE WITH YOUR SOLUTION
    # END Question 7
