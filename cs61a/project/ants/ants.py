        ...
        if insect.is_ant:
            ...
            if self.ant is insect:
                if hasattr(self.ant, 'container') and self.ant.container:
                    ...
                    if not isinstance(self.ant, QueenAnt) or self.ant.the_queen != 0:
                        self.ant = None
                        insect.place = None
            else:
                if hasattr(self.ant, 'container') and self.ant.container:
                    if self.ant.ant is insect:
                        if not isinstance(self.ant.ant, QueenAnt) or self.ant.ant.the_queen != 0:
                            #print("testt")
                            self.ant.ant = None
                            insect.place = None
                else:
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        """Returns true if this ant is a container and it does not already contain other ants, the other ant is not an container"""
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        in_range = True
        ind = 0
        while bee == None and nearest != None and in_range == True:
            ...
            if nearest is hive:
                ...
                if ind > self.min_range and ind < self.max_range:
                    bee = random_or_none(nearest.bees)
        return bee
        # END Problem 3B
...
# BEGIN Problem 5A
class WallAnt(Ant):
    """WallAnt does nothing, but it's large armor will keep bees busy """
    name = "Wall"
    food_cost = 4
    implemented = True
...
class ScubaThrower(ThrowerAnt):
    """ScubaThrower class is water safe thrower class"""
...
class BodyguardAnt(Ant):
    ...
    # why no instructions on putting contained ants back into place?
    #def reduce_armor(self, amount):
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1, place=None):
        ...
        self.armor = 1
        self.inspired = []
        self.the_queen = QueenAnt.the_queens
        QueenAnt.the_queens += 1
        # END Problem 9
    ...
    def action(self, colony):
        ...
        if self.the_queen == 0:
            ...
            plac = self.place.exit
            while plac != None: 
                if plac.ant != None and not (plac.ant in self.inspired):
                    plac.ant.damage = plac.ant.damage * 2
                    self.inspired.append(plac.ant)
                if hasattr(plac.ant, 'ant') and plac.ant.ant != None:
                    if (plac.ant.ant not in self.inspired):
                        plac.ant.ant.damage = plac.ant.ant.damage * 2
                        self.inspired.append(plac.ant.ant)
                plac = plac.exit
        else:
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self.the_queen == 0:
            bees_win()
        # END Problem 9
...
def apply_effect(effect, bee, duration):
    ...
    # BEGIN Problem EC
    #def old_be(colony):
    #    return old_bee = Bee.action(bee,colony)
    #old_bee = Bee(bee)
    
    def actio(colony):
        nonlocal duration
        #duration -= 1
        #new_bee = Bee.action(bee)
...
class SlowThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            #target.slow_duration = target.slow_duration + 3
...
class StunThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        if target:
            #target.stun_duration = target.stun_duration + 1
